...
def main(argv):
    ...
    clusterModule = loadClusterModule(args.pathClusteringCheckpoint, norm_vec_len=args.norm_vec_len)
    if not args.cpu:
        clusterModule.cuda()
    ...
    for index, vals in enumerate(seqNames):
        ...
        # Quantizing
        quantLine = quantize_file(file_path, cpc_feature_function, clusterModule, args)
