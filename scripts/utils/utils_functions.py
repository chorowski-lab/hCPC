...
def loadCPCFeatureMaker(pathCheckpoint, gru_level=-1, get_encoded=False, keep_hidden=True, load_nullspace=False, cpcLevel=0):
    """
    ...
    # Load CPC model
    model, nHiddenGar, nHiddenEncoder = loadModel([pathCheckpoint], updateConfig=updateConfig, load_nullspace=load_nullspace)
    ...
    featureMaker = FeatureModule(model, get_encoded=get_encoded, cpcLevel=cpcLevel)
...
def loadClusterModule(pathCheckpoint, norm_vec_len=False):
    ...
    clusterModule = kMeanCluster(torch.zeros(1, state_dict["n_clusters"], state_dict["dim"]), norm_vec_len=norm_vec_len)
